...
Key cleaning steps:
	•	Removed rows where group and landing_page did not match
(control ≠ new_page, treatment ≠ old_page).
...
\hat{p}_{new} - \hat{p}_{old} \approx -0.00158

The difference is slightly negative.
...
\text{logit}(conversion) = \beta_0 + \beta_1 \cdot ab\_page
